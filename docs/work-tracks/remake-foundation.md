...
Recommended next foundation item: `PET-DRAFT-SAVE-FLOW-CLIENT-001`.

Compose the shared pet draft client with the existing pet media upload/attach flow behind a platform-neutral draft save flow, with fake/injected dependencies first and no real UI or service calls.
