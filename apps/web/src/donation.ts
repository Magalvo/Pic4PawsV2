...
export const webDonationUiContent: WebDonationUiContent = {
  locale: 'pt-PT',
  status: 'product-flow-ready',
  title: 'Doação',
...
export type WebDonationIdleState = {
  state: 'idle';
  title: string;
  message: string;
...
export type WebDonationSubmittingState = {
  state: 'submitting';
  title: string;
  message: string;
};

export type WebDonationSubmittedState = {
  state: 'submitted';
  title: string;
  message: string;
  donationId: string;
...
export type WebDonationResultViewModel =
...
export const createWebDonationUi = ({
  donationClient,
}: {
  donationClient: Pick<DonationClient, 'submitDonation'>;
}) => ({
  getInitialState: (): WebDonationIdleState => ({
    state: 'idle',
    title: 'Fazer uma doação',
    message: 'Escolhe o valor e o método de pagamento para fazer a tua doação.',
    primaryAction: 'Doar',
