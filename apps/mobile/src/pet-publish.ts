...
export type MobilePetPublishUiState = 'ready' | 'publishing' | 'published' | 'failed';

export type MobilePetPublishUiStateCopy = {
  state: MobilePetPublishUiState;
  title: string;
  message: string;
};

export type MobilePetPublishUiContent = {
  locale: 'pt-PT';
  title: string;
  description: string;
  status: 'product-flow-ready';
  states: MobilePetPublishUiStateCopy[];
};

export type MobilePetPublishContext = {
  petId: string;
  petName: string;
};

export type MobilePetPublishReadyViewModel = {
  state: 'ready';
  title: string;
  message: string;
...
export type MobilePetPublishResultViewModel =
  | {
      state: 'published';
      title: string;
      message: string;
...
export type CreateMobilePetPublishUiInput = {
  publishClient: Pick<PetPublishClient, 'publishPetDraft'>;
};

export type MobilePetPublishUi = {
  getInitialState: (pet: MobilePetPublishContext) => MobilePetPublishReadyViewModel;
  publishPetDraft: (input: {
    pet: MobilePetPublishContext;
  }) => Promise<MobilePetPublishResultViewModel>;
};
...
export const mobilePetPublishUiContent: MobilePetPublishUiContent = {
  locale: 'pt-PT',
  title: 'Publicação do perfil',
  description:
    'Fluxo mobile para publicar um rascunho de animal através do Worker seguro, mantendo as regras de publicação no servidor.',
  status: 'product-flow-ready',
  states: [
    {
      state: 'ready',
...
export const createMobilePetPublishUi = ({
  publishClient,
}: CreateMobilePetPublishUiInput): MobilePetPublishUi => ({
  getInitialState: (pet) => ({
    state: 'ready',
