...
} from '../../apps/mobile/src/pet-publish';
...
describe('mobile pet publish UI flow', () => {
  it('exposes a PT-PT ready state for publishing a pet draft', () => {
    const ui = createMobilePetPublishUi({
      publishClient: {
    ...
    expect(mobilePetPublishUiContent.locale).toBe('pt-PT');
    expect(mobilePetPublishUiContent.states.map((state) => state.state)).toEqual([
      'ready',
      'publishing',
      'published',
      'failed',
  ...
  it('publishes a pet draft through the injected Mobile publish client', async () => {
  ...
  it('maps authorization, missing draft and worker failures to distinct safe Mobile copy', async () => {
  ...
  it('surfaces the pet publish product flow on the Mobile foundation content', () => {
    expect(mobileFoundationContent.petPublish.status).toBe('product-flow-ready');
    expect(mobileFoundationContent.petPublish.title).toBe('Publicação do perfil');
    expect(JSON.stringify(mobileFoundationContent.petPublish)).not.toContain('service-role');
    expect(JSON.stringify(mobileFoundationContent.petPublish)).not.toContain('r2-secret');
  });
});
