...
} from '../../apps/mobile/src/shelter-payment-config';
...
describe('mobileShelterPaymentConfigUiContent', () => {
  it('has pt-PT locale and product-flow-ready status', () => {
    expect(mobileShelterPaymentConfigUiContent.locale).toBe('pt-PT');
    expect(mobileShelterPaymentConfigUiContent.status).toBe('product-flow-ready');
  });

  it('has idle, saving, saved, failed, and forbidden states defined', () => {
    const stateNames = mobileShelterPaymentConfigUiContent.states.map((s) => s.state);
  ...
  it('does not expose credentials in content', () => {
    const content = JSON.stringify(mobileShelterPaymentConfigUiContent);
...
describe('mobileFoundationContent — shelterPaymentConfig entry', () => {
  it('includes shelterPaymentConfig with product-flow-ready status', () => {
    expect(mobileFoundationContent.shelterPaymentConfig.status).toBe('product-flow-ready');
    expect(mobileFoundationContent.shelterPaymentConfig.title).toBeTruthy();
  });
});

// ─── loadConfig ───────────────────────────────────────────────────────────────

describe('createMobileShelterPaymentConfigUi — loadConfig', () => {
  ...
  it('returns idle with null values when not configured', async () => {
    const ui = makeUi(
    ...
    if (state.state === 'idle') {
      ...
      expect(state.mbWayPhone).toBeNull();
    }
  });
...
describe('createMobileShelterPaymentConfigUi — saveConfig', () => {
