...
import {
  ...
  mobileAdoptionUiContent,
} from '../../apps/mobile/src/adoption';
...
describe('mobile adoption UI', () => {
  it('getInitialState returns idle state with PT-PT copy', () => {
    const ui = createMobileAdoptionUi({
      adoptionApplicationClient: {
    ...
    expect(mobileAdoptionUiContent.locale).toBe('pt-PT');
    expect(mobileAdoptionUiContent.status).toBe('product-flow-ready');
    expect(mobileAdoptionUiContent.states.map((s) => s.state)).toEqual([
      'idle',
      'submitting',
      'submitted',
      'pet_not_found',
  ...
  it('mobileAdoptionUiContent has pt-PT locale and all required states', () => {
    expect(mobileAdoptionUiContent.locale).toBe('pt-PT');
    expect(mobileAdoptionUiContent.status).toBe('product-flow-ready');

    const stateNames = mobileAdoptionUiContent.states.map((s) => s.state);
  ...
  it('mobile foundation content exposes adoptionApplication with product-flow-ready status', () => {
    expect(mobileFoundationContent.adoptionApplication.status).toBe('product-flow-ready');
    expect(mobileFoundationContent.adoptionApplication.title).toBeTruthy();
    expect(JSON.stringify(mobileFoundationContent.adoptionApplication)).not.toContain(
      'service-role',
    );
  });
});
