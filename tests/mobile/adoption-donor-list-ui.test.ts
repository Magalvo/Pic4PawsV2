...
} from '../../apps/mobile/src/adoption-donor-list';
...
describe('mobile adoption donor list UI', () => {
  it('getInitialState returns idle state', () => {
    const ui = createMobileAdoptionDonorListUi({ adoptionDonorListClient: makeClient(true) });
  ...
  it('loadDonorAdoptions returns empty state for zero applications', async () => {
    const ui = createMobileAdoptionDonorListUi({
  ...
  it('loadDonorAdoptions returns failed state on client error', async () => {
    const ui = createMobileAdoptionDonorListUi({ adoptionDonorListClient: makeClient(false) });
    const state = await ui.loadDonorAdoptions();
  ...
  it('mobileAdoptionDonorListUiContent locale is pt-PT', () => {
    expect(mobileAdoptionDonorListUiContent.locale).toBe('pt-PT');
  });

  it('mobileAdoptionDonorListUiContent status is product-flow-ready', () => {
    expect(mobileAdoptionDonorListUiContent.status).toBe('product-flow-ready');
  });

  it('mobile foundation content exposes adoptionDonorList with product-flow-ready status', () => {
    expect(mobileFoundationContent.adoptionDonorList.status).toBe('product-flow-ready');
    expect(mobileFoundationContent.adoptionDonorList.title).toBeTruthy();
    expect(mobileFoundationContent.adoptionDonorList.description).toBeTruthy();
  });
});
