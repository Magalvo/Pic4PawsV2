...
} from '../../apps/mobile/src/notification-preferences';
...
describe('mobile notification preferences UI', () => {
  it('getInitialState returns idle state', () => {
    const ui = createMobileNotificationPreferencesUi({
    ...
    expect(mobileNotificationPreferencesUiContent.locale).toBe('pt-PT');
    expect(mobileNotificationPreferencesUiContent.status).toBe('product-flow-ready');
  });
  ...
  it('mobileNotificationPreferencesUiContent has pt-PT locale and product-flow-ready status', () => {
    expect(mobileNotificationPreferencesUiContent.locale).toBe('pt-PT');
    expect(mobileNotificationPreferencesUiContent.status).toBe('product-flow-ready');
    expect(mobileNotificationPreferencesUiContent.title).toBeTruthy();
  });

  it('mobile foundation content exposes notificationPreferences with product-flow-ready status', () => {
    expect(mobileFoundationContent.notificationPreferences.status).toBe('product-flow-ready');
    expect(mobileFoundationContent.notificationPreferences.title).toBeTruthy();
    expect(JSON.stringify(mobileFoundationContent.notificationPreferences)).not.toContain('service-role');
  });
});
