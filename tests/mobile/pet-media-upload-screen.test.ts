...
import { createMobilePetMediaUploadUi } from '../../apps/mobile/src/pet-media-upload';
...
describe('pet media upload screen — boundary contract', () => {
  it('produces uploaded state on success', async () => {
    const ui = createMobilePetMediaUploadUi({ uploadAttachFlow: makeClient(true) });
  ...
  it('produces failed state on upload_intent error', async () => {
    const ui = createMobilePetMediaUploadUi({ uploadAttachFlow: makeClient(false) });
    const result = await ui.uploadSelectedImage({ pet: samplePet, file: sampleFile });
  ...
  it('failed state does not expose bearer or service-role in reasons', async () => {
    ...
    const ui = createMobilePetMediaUploadUi({ uploadAttachFlow: poisonClient });
    const result = await ui.uploadSelectedImage({ pet: samplePet, file: sampleFile });
