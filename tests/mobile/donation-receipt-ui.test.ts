...
} from '../../apps/mobile/src/donation-receipt';
...
const makeFile = (): MobileDonationReceiptFileInput => ({
  uri: 'file:///data/photos/receipt.jpg',
  type: 'image/jpeg',
  name: 'receipt.jpg',
  size: 1024,
});
...
describe('mobileDonationReceiptUiContent', () => {
  it('has pt-PT locale and product-flow-ready status', () => {
    expect(mobileDonationReceiptUiContent.locale).toBe('pt-PT');
    expect(mobileDonationReceiptUiContent.status).toBe('product-flow-ready');
  });

  it('has all required states defined', () => {
    const stateNames = mobileDonationReceiptUiContent.states.map((s) => s.state);
  ...
  it('does not expose credentials in content', () => {
    const content = JSON.stringify(mobileDonationReceiptUiContent);
...
describe('mobileFoundationContent — donationReceipt entry', () => {
  it('includes donationReceipt with product-flow-ready status', () => {
    expect(mobileFoundationContent.donationReceipt.status).toBe('product-flow-ready');
    expect(mobileFoundationContent.donationReceipt.title).toBeTruthy();
  });
});

// ─── loadDonation ─────────────────────────────────────────────────────────────

describe('createMobileDonationReceiptUi — loadDonation', () => {
...
describe('createMobileDonationReceiptUi — uploadAndSubmit', () => {
