...
describe('pet draft save flow screen — boundary contract', () => {
  it('produces saved state on success', async () => {
    const ui = createMobilePetDraftSaveFlowUi({ saveFlowClient: makeClient(true) });
  ...
  it('produces failed state on draft_save error', async () => {
    const ui = createMobilePetDraftSaveFlowUi({ saveFlowClient: makeClient(false) });
    const result = await ui.saveDraft({ context: { petName: 'Bolinha' }, draft: sampleDraft });
  ...
  it('produces failed state on media_upload error', async () => {
    ...
    const ui = createMobilePetDraftSaveFlowUi({ saveFlowClient: mediaFailClient });
    const result = await ui.saveDraft({ context: { petName: 'Bolinha' }, draft: sampleDraft });
    expect(result.state).toBe('failed');
    if (result.state === 'failed') {
  ...
  it('failed state does not expose bearer or service-role in reasons', async () => {
    ...
    const ui = createMobilePetDraftSaveFlowUi({ saveFlowClient: poisonClient });
    const result = await ui.saveDraft({ context: { petName: 'Bolinha' }, draft: sampleDraft });
    const serialized = JSON.stringify(result).toLowerCase();
    expect(serialized).not.toContain('service-role');
    expect(serialized).not.toContain('bearer ');
