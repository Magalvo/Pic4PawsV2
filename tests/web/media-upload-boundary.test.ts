...
} from '../../apps/web/src/media-upload';
...
describe('web media upload boundary', () => {
  it('uploads a public pet image through injected dependencies and safe PT-PT states', async () => {
    ...
    const boundary = createWebMediaUploadBoundary({
  ...
  it('maps intent failures to safe Portuguese Web copy', async () => {
    const boundary = createWebMediaUploadBoundary({
      workerBaseUrl: 'https://worker.pic4paws.pt',
      mediaUploadPath: '/uploads/media',
      getAccessToken: async () => 'user-access-token',
  ...
  it('surfaces media upload readiness on the Portuguese foundation screen', () => {
    expect(webMediaUploadContent.locale).toBe('pt-PT');
    expect(webMediaUploadContent.states.map((state) => state.state)).toEqual([
      'idle',
      'uploading',
      'uploaded',
      'intent_failed',
    ...
    expect(webFoundationContent.mediaUpload.status).toBe('contract-ready');
    expect(webFoundationContent.mediaUpload.title).toBe('Upload seguro de imagens');
    expect(JSON.stringify(webFoundationContent.mediaUpload)).not.toContain('signedUrl');
  });
});
