...
import { createWebNotificationPreferencesUi } from '../../apps/web/src/notification-preferences';
import type {
...
const preferences = [{ type: 'donation_paid' as const, enabled: true }];

describe('web notification preferences page — boundary contract', () => {
  it('produces loaded state with preferences', async () => {
    ...
    if (result.state === 'loaded') expect(result.preferences).toHaveLength(1);
  });
  ...
  it('produces loaded state on successful toggle', async () => {
    const updatedPrefs = [{ type: 'donation_paid' as const, enabled: false }];
    const client = makeClient({
      load: { ok: true, status: 'ok', preferences },
      update: { ok: true, status: 'ok', preferences: updatedPrefs },
    });
    const ui = createWebNotificationPreferencesUi({ notificationPreferencesClient: client });
