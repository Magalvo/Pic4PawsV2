...
import {
  createWebShelterMemberUi,
  webShelterMemberUiContent,
} from '../../apps/web/src/shelter-member';
...
describe('web shelter member UI', () => {
  it('getInitialState returns idle state with PT-PT copy', () => {
    const ui = createWebShelterMemberUi({
      shelterMemberClient: {
    ...
    expect(webShelterMemberUiContent.locale).toBe('pt-PT');
    expect(webShelterMemberUiContent.status).toBe('product-flow-ready');
  });

  it('webShelterMemberUiContent has all 8 required states', () => {
    const stateNames = webShelterMemberUiContent.states.map((s) => s.state);
  ...
  it('webShelterMemberUiContent has pt-PT locale and product-flow-ready status', () => {
    expect(webShelterMemberUiContent.locale).toBe('pt-PT');
    expect(webShelterMemberUiContent.status).toBe('product-flow-ready');
    expect(webShelterMemberUiContent.title).toBeTruthy();
    expect(webShelterMemberUiContent.description).toBeTruthy();
  });

  it('web foundation content exposes shelterMember with product-flow-ready status', () => {
    expect(webFoundationContent.shelterMember.status).toBe('product-flow-ready');
    expect(webFoundationContent.shelterMember.title).toBeTruthy();
    expect(webFoundationContent.shelterMember.description).toBeTruthy();
    expect(JSON.stringify(webFoundationContent.shelterMember)).not.toContain('service-role');
  });
});
