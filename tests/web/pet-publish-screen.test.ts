...
import { createWebPetPublishUi } from '../../apps/web/src/pet-publish';
...
describe('pet publish page — boundary contract', () => {
  it('produces published state on success', async () => {
    const ui = createWebPetPublishUi({ publishClient: makeClient(true) });
  ...
  it('produces failed state on error', async () => {
    const ui = createWebPetPublishUi({ publishClient: makeClient(false) });
    const result = await ui.publishPetDraft({ pet: samplePet });
    expect(result.state).toBe('failed');
  });

  it('passes petId to publishPetDraft', async () => {
    ...
    const ui = createWebPetPublishUi({ publishClient: trackingClient });
  ...
  it('failed state does not expose bearer or service-role in reasons', async () => {
    ...
    const ui = createWebPetPublishUi({ publishClient: poisonClient });
    const result = await ui.publishPetDraft({ pet: samplePet });
