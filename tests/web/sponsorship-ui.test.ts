...
import {
  ...
  webSponsorshipUiContent,
} from '../../apps/web/src/sponsorship';
...
describe('web sponsorship UI', () => {
  it('getInitialState returns idle state with PT-PT copy and primaryAction', () => {
    ...
    expect(webSponsorshipUiContent.locale).toBe('pt-PT');
    expect(webSponsorshipUiContent.status).toBe('product-flow-ready');
  });

  it('webSponsorshipUiContent has all 4 required states', () => {
    const stateNames = webSponsorshipUiContent.states.map((s) => s.state);
  ...
  it('webSponsorshipUiContent has pt-PT locale and product-flow-ready status', () => {
    expect(webSponsorshipUiContent.locale).toBe('pt-PT');
    expect(webSponsorshipUiContent.status).toBe('product-flow-ready');
    expect(webSponsorshipUiContent.title).toBeTruthy();
    expect(webSponsorshipUiContent.description).toBeTruthy();
  });

  it('web foundation content exposes sponsorship with product-flow-ready status', () => {
    expect(webFoundationContent.sponsorship.status).toBe('product-flow-ready');
    expect(webFoundationContent.sponsorship.title).toBeTruthy();
    expect(JSON.stringify(webFoundationContent.sponsorship)).not.toContain('service-role');
  });
});
