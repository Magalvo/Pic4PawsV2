...
import {
  ...
  webAdoptionListUiContent,
} from '../../apps/web/src/adoption-list';
...
describe('web adoption list UI', () => {
  it('getInitialState returns idle state with PT-PT copy and content contract', () => {
    ...
    expect(webAdoptionListUiContent.locale).toBe('pt-PT');
    expect(webAdoptionListUiContent.status).toBe('product-flow-ready');
    expect(webAdoptionListUiContent.states.map((s) => s.state)).toEqual(
  ...
  it('web foundation content exposes adoptionList with product-flow-ready status', () => {
    expect(webFoundationContent.adoptionList.status).toBe('product-flow-ready');
    expect(webFoundationContent.adoptionList.title).toBeTruthy();
    expect(JSON.stringify(webFoundationContent.adoptionList)).not.toContain('service-role');
  });
});
