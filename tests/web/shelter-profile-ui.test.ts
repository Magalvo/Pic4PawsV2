...
import {
  ...
  webShelterProfileUiContent,
} from '../../apps/web/src/shelter-profile';
...
describe('web shelter profile UI', () => {
  it('getInitialState returns idle state with PT-PT copy', () => {
    const ui = createWebShelterProfileUi({
      shelterProfileClient: {
        loadProfile: async () => {
    ...
    expect(webShelterProfileUiContent.locale).toBe('pt-PT');
    expect(webShelterProfileUiContent.status).toBe('product-flow-ready');
    expect(webShelterProfileUiContent.states.map((s) => s.state)).toEqual([
      'idle',
      'loading',
      'loaded',
      'not_found',
  ...
  it('webShelterProfileUiContent has pt-PT locale and all required states', () => {
    expect(webShelterProfileUiContent.locale).toBe('pt-PT');
    expect(webShelterProfileUiContent.status).toBe('product-flow-ready');

    const stateNames = webShelterProfileUiContent.states.map((s) => s.state);
  ...
  it('web foundation content exposes shelterProfile with product-flow-ready status', () => {
    expect(webFoundationContent.shelterProfile.status).toBe('product-flow-ready');
    expect(webFoundationContent.shelterProfile.title).toBeTruthy();
    expect(JSON.stringify(webFoundationContent.shelterProfile)).not.toContain('service-role');
  });
});
