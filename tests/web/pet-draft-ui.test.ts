...
import { createWebPetDraftUi, webPetDraftUiContent } from '../../apps/web/src/pet-draft';
import type {
  PetDraftClient,
...
describe('web pet draft UI flow', () => {
  it('exposes a PT-PT ready state for editing a pet draft', () => {
    const ui = createWebPetDraftUi({
      draftClient: {
    ...
    expect(webPetDraftUiContent.locale).toBe('pt-PT');
    expect(webPetDraftUiContent.states.map((state) => state.state)).toEqual([
      'ready',
      'saving',
      'saved',
      'failed',
  ...
  it('creates pet drafts through the injected Web draft client with safe payload only', async () => {
  ...
  it('updates pet drafts through the injected Web draft client with safe payload only', async () => {
    const calls: PetDraftClientDraftInput[] = [];
    const draftClient: PetDraftClient = {
      createPetDraft: async () => {
  ...
  it('maps authorization and worker failures to distinct safe Web copy', async () => {
  ...
  it('surfaces the pet draft product flow on the Web foundation content', () => {
    expect(webFoundationContent.petDraft.status).toBe('product-flow-ready');
    expect(webFoundationContent.petDraft.title).toBe('Rascunho do perfil');
    expect(JSON.stringify(webFoundationContent.petDraft)).not.toContain('service-role');
    expect(JSON.stringify(webFoundationContent.petDraft)).not.toContain('r2-secret');
  });
});
