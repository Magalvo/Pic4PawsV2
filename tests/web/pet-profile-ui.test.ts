...
import {
  createWebPetProfileUi,
  webPetProfileUiContent,
} from '../../apps/web/src/pet-profile';
...
describe('web pet profile UI', () => {
  it('getInitialState returns idle state with PT-PT copy', () => {
    const ui = createWebPetProfileUi({
      profileClient: {
        loadProfile: async () => {
    ...
    expect(webPetProfileUiContent.locale).toBe('pt-PT');
    expect(webPetProfileUiContent.status).toBe('product-flow-ready');
    expect(webPetProfileUiContent.states.map((s) => s.state)).toEqual([
      'idle',
      'loading',
      'loaded',
      'not_found',
  ...
  it('webPetProfileUiContent has pt-PT locale and all required states', () => {
    expect(webPetProfileUiContent.locale).toBe('pt-PT');
    expect(webPetProfileUiContent.status).toBe('product-flow-ready');

    const stateNames = webPetProfileUiContent.states.map((s) => s.state);
  ...
  it('web foundation content exposes petProfile with product-flow-ready status', () => {
    expect(webFoundationContent.petProfile.status).toBe('product-flow-ready');
    expect(webFoundationContent.petProfile.title).toBeTruthy();
    expect(JSON.stringify(webFoundationContent.petProfile)).not.toContain('service-role');
  });
});
