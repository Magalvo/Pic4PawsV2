...
} from '../../apps/web/src/sponsorship-list';
...
describe('web sponsorship list UI', () => {
  it('getInitialState returns idle state with PT-PT copy', () => {
    const ui = createWebSponsorshipListUi({
      sponsorshipListClient: {
    ...
    expect(webSponsorshipListUiContent.locale).toBe('pt-PT');
    expect(webSponsorshipListUiContent.status).toBe('product-flow-ready');
  });

  it('webSponsorshipListUiContent has all 6 required states', () => {
    const stateNames = webSponsorshipListUiContent.states.map((s) => s.state);
  ...
  it('webSponsorshipListUiContent has pt-PT locale and product-flow-ready status', () => {
    expect(webSponsorshipListUiContent.locale).toBe('pt-PT');
    expect(webSponsorshipListUiContent.status).toBe('product-flow-ready');
    expect(webSponsorshipListUiContent.title).toBeTruthy();
    expect(webSponsorshipListUiContent.description).toBeTruthy();
  });

  it('web foundation content exposes sponsorshipList with product-flow-ready status', () => {
    expect(webFoundationContent.sponsorshipList.status).toBe('product-flow-ready');
    expect(webFoundationContent.sponsorshipList.title).toBeTruthy();
    expect(webFoundationContent.sponsorshipList.description).toBeTruthy();
    expect(JSON.stringify(webFoundationContent.sponsorshipList)).not.toContain('service-role');
  });
});
