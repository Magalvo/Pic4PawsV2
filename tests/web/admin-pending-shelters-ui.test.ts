...
import {
  ...
  webAdminPendingSheltersUiContent,
} from '../../apps/web/src/admin-pending-shelters';
import type {
...
describe('webAdminPendingSheltersUiContent', () => {
  it('has pt-PT locale and product-flow-ready status', () => {
    expect(webAdminPendingSheltersUiContent.locale).toBe('pt-PT');
    expect(webAdminPendingSheltersUiContent.status).toBe('product-flow-ready');
  });

  it('defines idle, loaded, empty, forbidden, and failed states', () => {
    const states = webAdminPendingSheltersUiContent.states.map((state) => state.state);
  ...
  it('does not expose credential markers in static content', () => {
    const serialized = JSON.stringify(webAdminPendingSheltersUiContent);
    expect(serialized).not.toContain('service-role');
    expect(serialized).not.toContain('bearer ');
  });
});

describe('createWebAdminPendingSheltersUi', () => {
  it('returns an idle initial state with PT-PT copy', () => {
    const ui = createWebAdminPendingSheltersUi({
  ...
  it('passes query through to the client unchanged', async () => {
    ...
    const ui = createWebAdminPendingSheltersUi({ adminPendingSheltersClient: client });
